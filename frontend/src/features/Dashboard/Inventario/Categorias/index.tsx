...
export default function Categorias() {
  ...
  const [formErrors, setFormErrors] = useState<{ nombre?: string; descripcion?: string; tipo?: string }>({});
  ...
  async function fetchCategorias() {
    ...
    try {
      ...
      const resp = await api.get('/dashboard/table-data/categoria_insumo?limit=1000&order=id_categoria.asc');
  ...
  async function submitForm(e: React.FormEvent) {
    ...
    try {
      if (editing) {
        ...
        const payload: { nombre: string; descripcion?: string; tipo_categoria: string | null } = { 
          nombre: formNombre.trim(), 
          descripcion: formDescripcion.trim() || undefined,
          tipo_categoria: formTipo || null 
        };
            ...
            <tbody className="text-gray-800">
              ...
              {rows
                .filter((r) => {
                  ...
                  if (q && !(`${r.nombre}`.toLowerCase().includes(q.toLowerCase()) || `${r.descripcion}`.toLowerCase().includes(q.toLowerCase()) || `${r.tipo_categoria}`.toLowerCase().includes(q.toLowerCase()))) return false;
                  return true;
                })
                .map((r) => (
              ...
              <div className="p-5">
                <form onSubmit={submitForm}>
                  ...
                  <div className="grid gap-3">
                    ...
                    <div className={`${formErrors.tipo ? 'p-3 border border-red-300 rounded-lg bg-red-50' : ''}`}>
                      <label className="block text-xs text-gray-600 mb-1">
                        Tipo de categoría <span className="text-red-500">*</span>
                      </label>
                      {viewing ? (
                        <input value={formTipo} className="w-full h-11 rounded-lg border px-3 bg-gray-50 text-sm" readOnly />
                      ) : (
